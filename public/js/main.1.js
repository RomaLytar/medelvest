...
$(window).on('scroll', function () {
  ...
  let slider;
  if ($(window).width() > 992) {
    slider = $('.secondConstructorSlider ');
    if (this.scrollY > 350) {
...

{
  ...
  $('.customSizes input').on('keyup', function (event) {
  ...
  function changeCommonPrice() {
    ...
    if ($(this).parent().parent().hasClass('tab_fasad')) {
      ...
      $('#premiumFasad .price-value').removeClass('active');
      if ($(this).parent().parent().parent().attr('id') == 'premiumFasad') {
  ...
  function closeSecondsAccordionitem() {
    $('.second-config-tab').next().next().removeClass('open');
    $('.second-config-tab').next().next().next().slideUp("slow");
    $('.addEquipWrapp').removeClass('open');
    $('.addEquipWrapp').next().slideUp("slow");
  };
